...
class EngineMetadata:
    ...
    def is_compatible_with(
        self, new_shapes: Dict[str, Any], new_extra_args: Set[str] = set()
    ) -> bool:
...
class EngineMetadataManager:
    ...
    def save_metadata(
        ...
        try:
            ...
            if self.verbose:
                print(f"[INFO] Saved engine metadata: {metadata_path}")

        except Exception as e:
            print(
                f"[WARN] Failed to save metadata for {engine_path}: {e}, "
                "unnecessary recompilations may occur."
            )
    ...
    def check_engine_compatibility(
        ...
        try:
            ...
            # Check static_shape compatibility
            saved_static_shape = metadata.get("static_shape", True)
            if saved_static_shape != static_shape:
                saved_strategy = "static" if saved_static_shape else "dynamic"
                required_strategy = "static" if static_shape else "dynamic"
                return (
                    False,
                    f"Shape strategy mismatch: existing engine is {saved_strategy}, user requested {required_strategy}",
                )
            ...
            if static_shape:
                ...
                for input_name, target_shape in target_shapes.items():
                    ...
                    if (
                        ...
                        if (
                            target_min != min_shape
                            or target_opt != opt_shape
                            or target_max != max_shape
                        ):
                            return False, f"Dynamic shape range mismatch for '{input_name}'"
                    else:
                        # Target is single shape - check if it fits within bounds
                        target_shape = (
                            tuple(target_shape) if isinstance(target_shape, list) else target_shape
                        )
                        ...
                        for i, (target_dim, min_dim, max_dim) in enumerate(
                            zip(target_shape, min_shape, max_shape)
                        ):
    ...
    def cleanup_metadata(self, engine_path: Path) -> None:
        ...
        if metadata_path.exists():
            ...
            if self.verbose:
                print(f"[INFO] Removed metadata: {metadata_path}")

...
class BaseModel:
    ...
    def validate_io_names(
        self, precision: str, input_names: list, output_names: Optional[list] = None
    ) -> None:
        ...
        assert set(input_names) == set(expected_inputs), (
            f"Input name mismatch for {self.name}[{precision}]: "
            f"expected {expected_inputs}, got {input_names}"
        )

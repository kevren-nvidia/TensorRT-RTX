...
MODELS: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    "ltx_video_transformer_2b_txt2vid": {
        "bf16": {
            ...
            "input_shapes": {
                ...
                "timestep": ("2B",),
                "encoder_attention_mask": ("2B", 128),
                "image_rotary_emb_cos": ("2B", "hidden_dim", 2048),
                "image_rotary_emb_sin": ("2B", "hidden_dim", 2048),
            },
            "input_dtypes": {
                "hidden_states": trt.DataType.BF16,
                "encoder_hidden_states": trt.DataType.BF16,
                "timestep": trt.DataType.FLOAT,
                "encoder_attention_mask": trt.DataType.BOOL,
                "image_rotary_emb_cos": trt.DataType.FLOAT,
                "image_rotary_emb_sin": trt.DataType.FLOAT,
            },
            "output_shapes": {
                "latent": ("2B", "hidden_dim", 128),
            },
        },
        "fp8": {
    ...
    "ltx_video_vae_2b_encoder": {
        "fp16": {
            ...
            "output_shapes": {
                "latent_image": (1, 3, 1, "latent_height", "latent_width"),
            },
        }
    },
    "ltx_video_vae_2b_decoder": {
        "fp16": {
            ...
            "input_dtypes": {"latent": trt.DataType.HALF},
            "output_shapes": {
                "video": ("B", 3, "num_frames", "height", "width"),
            },
        }
    },
}
...
PIPELINES: Dict[str, Dict[str, str]] = {
    "ltx_video_2b_txt2vid": {
        "transformer": "ltx_video_transformer_2b_txt2vid",
        "vae_decoder": "ltx_video_vae_2b_decoder",
        "text_encoder": "ltx_video_t5_text_encoder",
    },
    "ltx_video_2b_img2vid": {
        "transformer": "ltx_video_transformer_2b_img2vid",
        "vae_decoder": "ltx_video_vae_2b_decoder",
        "vae_encoder": "ltx_video_vae_2b_encoder",
        "text_encoder": "ltx_video_t5_text_encoder",
    },
}
...
DEFAULT_PRECISIONS: Dict[str, Dict[str, str]] = {
    "ltx_video_2b_txt2vid": {"transformer": "bf16", "vae_decoder": "fp16", "text_encoder": "bf16"},
    "ltx_video_2b_img2vid": {
        "transformer": "bf16",
        "vae_decoder": "fp16",
        "vae_encoder": "fp16",
...
class ModelRegistry:
    ...
    def get_model_config(
        self, pipeline_name: str, role: str, precision: str
    ...
    def get_onnx_url(self, pipeline_name: str, role: str, precision: str) -> Optional[str]:
        """
        Get ONNX source (URL or local path) for a model role.

        The onnx_url can be either:
        - A path to a remote model directory or snapshot, sourced from the parent pipeline configuration
        - A local file path (e.g., "./dev_models/model.onnx") for copying during development

        Both are treated the same way by PathManager.acquire_onnx_file()
        ...
        return config.get("onnx_url")

    def validate_precision_config(
        self, pipeline_name: str, precision_config: Dict[str, str]
    ) -> Dict[str, str]:
        ...
        for role in pipeline_roles:
            if role in precision_config:
                ...
                if precision_config[role] in available_precisions:
                    ...
                    print(
                        f"[WARN] Precision '{precision_config[role]}' not available for {role}, using default"
                    )
    ...
    def print_available_models(self, pipeline_name: str):
        ...
        print(f"\n[INFO] Available models for {pipeline_name}:")
        ...
        for role in roles:
            ...
            print(f"[INFO]   {role} ({model_id}): {precisions} (default: {default})")

    def print_sharing_info(self):
        """Print model sharing information"""
        print("\n[INFO] Model Sharing Analysis:")
        ...
        if shared_models:
            print(f"[INFO] Shared models (cached once, linked across pipelines):")
            for model_id, usage in shared_models.items():
                print(f"[INFO]   {model_id}: {', '.join(usage)}")

        if unique_models:
            print("[INFO] Pipeline-specific models:")
            for model_id, usage in unique_models.items():
                print(f"[INFO]   {model_id}: {usage[0]}")
